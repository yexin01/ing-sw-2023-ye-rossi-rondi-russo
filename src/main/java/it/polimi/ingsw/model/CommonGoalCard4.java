...
public class CommonGoalCard4 extends CommonGoalCard {
    ...
    /**
     * Goal4: "Six separate groups, each formed by two adjacent tiles of the same type (not necessarily as shown in the figure).
    ...
    private int processGroup(ItemTile[][] mat, int[][] checkable, int x, int y) {
        ...
        for (int[] direction : directions) {
            ...
            if (isValidPosition(mat,newX,newY) && mat[newX][newY].getType().equals(type)) {
                ...
                markGroup(checkable,newX,newY);
                break;
            }
        }
        if (groupPositions.size()>=4) {
            return 1;
        }
        return 0;
    }
     ...
     * @param checkable matrix of checkable elements
     * @param x of the position found
     * @param y of the position found
     */
    private void markGroup(int[][] checkable, int x, int y) {
        checkable[x][y] = 0;
    }
